...
class Trainer:
    def __init__(
        ...
        self.epochs= 0   
        self.loss_fn=  LearnedPerceptualImagePatchSimilarity(net_type="alex").to(self.gpu_id)
    ...
    def _run_batch(self, y):
        ...
        timesteps = torch.randint(0, self.num_time_steps - 1, (y.shape[0],), device=self.gpu_id)

        #timesteps =  self.timesteps_schedule(num_time_steps=self.num_time_steps-1,num_samples=y.shape[0])   
    ...
    def _run_epoch(self, epoch): 
        ...
        for x, y in self.train_data:  
            ...
            result=  'LPIPS Loss: {:.4f}\tTraining Step: {:7d}/{:7d}\tNumber of Time Steps: {:7d}\tMin noise index: {:5d}\tMin sigma: {:5f}\tMax noise index: {:5d}\tMax sigma: {:5f}\tBeta: {:2f}\tAlpha: {:2f}\tLR: {:2f} \tSigma Data: {:2f}\tBase Channels: {:4d} \tBatch Step: {:4d}/{:4d}\tEpoch: {:5d}/{:5d}\tGpu ID: {:3d}\tMemory: {:.2f}/{:.2f}\tVersion: {}\tTime: {}'.format(
    ...
    def loss_fun_improved(self ,current_noisy_data,next_noisy_data,current_sigmas,next_sigmas  ): 
        ...
        next_y=next_y.repeat(1,3,1,1)
        current_y=current_y.repeat(1,3,1,1)
        lpips_loss = self.loss_fn(
            next_y.clamp(-1.0, 1.0), current_y.clamp(-1.0, 1.0)
        )
        overflow_loss = F.mse_loss(
            next_y, next_y.detach().clamp(-1.0, 1.0)
        ) 
        return lpips_loss + overflow_loss 
    ...
    def timesteps_schedule( self,   current_training_step
        ) -> int: 
        ...
        num_timesteps = math.ceil(math.sqrt(num_timesteps + self.initial_timesteps**2) - 1)
