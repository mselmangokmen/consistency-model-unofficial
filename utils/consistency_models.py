...
def beta_timestep_distribution(   num_samples, num_time_steps,  alpha=5,beta=1.5 ) :
 
        values = np.random.beta(alpha, beta, num_samples)  
...
class ImprovedConsistencyTraining:
    ...
    def __call__(
        ...
        **kwargs : Any
        ...
        noise = torch.randn_like(x)
        if self.dist_type=='imp':
            timesteps = lognormal_timestep_distribution(
                x.shape[0], sigmas, self.lognormal_mean, self.lognormal_std
            )
        else:  
            timesteps = beta_timestep_distribution(
               num_samples =  x.shape[0], num_time_steps=num_timesteps-1, alpha=self.alpha, beta=self.beta
            )
